...
async def check_routing(state: LawState) -> dict:
    """Route clear specialist topics without another sequential LLM call.

    The Law Agent already has the original question, so keyword routing is
    sufficient for the codelab domains. Ambiguous regulatory language is routed
    to Compliance conservatively rather than paying for a separate classifier.
    """
    ...
    messages = [
        SystemMessage(
            content=(
                ...
                "Keep the final response under 600 words."
            )
        ),
